...
import VerCita from '@/components/Admin/Modales/EditarCita';
...
const index = ({ user, medicos }) => {
  ...
  const [modalCreacionCita, setmodalCreacionCita] = useState(false);
  ...
  /*------------Sección para el envio de SMS y correos--------- */
    ...
    <AdminLayout>
      ...
      {modalInfoCita && (
        <VerCita
          ...
          user={user}
          medicos={medicos}
          date={parseFechaPgsql(selectedDate)}
      ...
      />
      {/*Modal para la creación de citas */}
      <ModalCreacion
        show={modalCreacionCita}
        handleClose={() => {
          setmodalCreacionCita(false);
        }}
        user={user}
        date={parseFechaPgsql(selectedDate)}
        medicos={medicos}
